...
Vue.component('contentManagementEditInformation', {
    components: {
...
function loadInfoContent(vue) {
    makeServerCall('GET', '/ContentManagement/Information/GetInfoContent/' + window.location.pathname.split('/').pop(), null, (ResultData) => {
        vue.Info = Object.assign(new Information(), ResultData);
        vue.loadedData.push(true);

        if (vue.Info.Files && vue.Info.Files.length) {
            vue.table.loading = true;

            let counter = 1;
            vue.Info.Files.forEach(x => {
                makeServerCall('POST', '/AttachedDocument/GetAttachedDocumentDetails', { attDocumentGUID: x }, (response) => {
                    vue.filesInfo.push(response);

                    if (counter == vue.Info.Files.length) vue.table.loading = false;
                    counter++;
                });
            });
        }
